...
const AddQuotation = () => {
  ...
  // Prefill from convertLead (if present) when navigated from Lead -> Convert to Quot
  useEffect(() => {
    try {
      const raw = localStorage.getItem("convertLead");
      if (raw) {
        const lead = JSON.parse(raw);
        setFormData(prev => ({
          ...prev,
          customerName: lead.name || lead.customerName || prev.customerName,
          email: lead.email || prev.email,
          phone: (lead.phone || lead.mobile || lead.phoneNumber || "").toString().slice(0, 10),
          address: lead.address || prev.address,
          state: lead.state || prev.state,
        }));
        localStorage.removeItem("convertLead");
      }
    } catch (e) {
      console.warn("Failed to prefill quotation from convertLead:", e);
    }
  }, []);
