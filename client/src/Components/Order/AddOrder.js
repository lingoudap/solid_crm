...
const AddOrder = () => {
  ...
  useEffect(() => {
    try {
      const raw = localStorage.getItem("convertQuote");
      if (raw) {
        const quote = JSON.parse(raw);
        setFormData(prev => ({
          ...prev,
          customerName: quote.customerName || quote.name || prev.customerName,
          email: quote.email || prev.email,
          phone: (quote.phone || quote.mobile || "").toString().slice(0, 10),
          address: quote.address || prev.address,
          state: quote.state || prev.state,
        }));
        if (Array.isArray(quote.items) && quote.items.length > 0) {
          const mapped = quote.items.map(it => ({
      ...
      console.warn("Failed to prefill order from convertQuote:", e);
    }
  }, []);
